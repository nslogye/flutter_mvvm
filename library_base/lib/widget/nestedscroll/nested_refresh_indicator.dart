...
class NestedRefreshIndicatorState extends State<NestedRefreshIndicator>
    with TickerProviderStateMixin<NestedRefreshIndicator> {
  ...
  bool _shouldStart(ScrollNotification notification) {
    return (notification is ScrollStartNotification ||
            (notification is ScrollUpdateNotification &&
                ...
                widget.triggerMode ==
                    NestedRefreshIndicatorTriggerMode.anywhere)) &&
  ...
  void _checkDragOffset(double containerExtent) {
    assert(_mode == _NestedRefreshIndicatorMode.drag ||
        _mode == _NestedRefreshIndicatorMode.armed);
    double newValue =
  ...
  Future<void> _dismiss(_NestedRefreshIndicatorMode newMode) async {
    ...
    // direction change.
    assert(newMode == _NestedRefreshIndicatorMode.canceled ||
        newMode == _NestedRefreshIndicatorMode.done);
    setState(() {
      _mode = newMode;
    });
    switch (_mode) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool showIndeterminateIndicator =
        _mode == _NestedRefreshIndicatorMode.refresh ||
            _mode == _NestedRefreshIndicatorMode.done;
...
bool nestedScrollNotificationPredicate(ScrollNotification notification) {
  return true;
}
