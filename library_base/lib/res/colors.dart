/*
 * @Author: wanku.ye nslogye@gmail.com
 * @Date: 2025-06-03 15:38:36
 * @LastEditors: wanku.ye nslogye@gmail.com
 * @LastEditTime: 2025-06-11 16:04:33
 * @FilePath: /flutter_mvvm/library_base/lib/res/colors.dart
 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
 */
import 'package:flutter/material.dart';

class Colours {
  static const Color app_main = Color(0xFF121826);
  static const Color app_main_500 = Color(0xFF121826);
  static const Color dark_app_main = Color(0xFF121826);

  static const Color normal_bg = Color(0xFFFFFFFF);

  static const Color toast_bg = Color(0xffffffff);
  static const Color toast_error = Color(0xffF46363);
  static const Color toast_success = Color(0xff2ED477);
  static const Color toast_warn = Color(0xffFF9138);
  static const Color toast_shadow_dark = Color(0x1028293D);
  static const Color toast_shadow_light = Color(0x20606170);

  static const Color gray_50 = Color(0xffF0F4FA);
  static const Color gray_100 = Color(0xffF4F5FB);
  static const Color gray_150 = Color(0xffE8EFF7);
  static const Color gray_200 = Color(0xffD6DCE4);
  static const Color gray_300 = Color(0xffC0C9DF);
  static const Color gray_350 = Color(0xffB2B9CD);
  static const Color gray_400 = Color(0xff8797AB);
  static const Color gray_500 = Color(0xff6C7F99);
  static const Color gray_600 = Color(0xff677587);
  static const Color gray_800 = Color(0xff333A50);

  static const Color normal_border_shadow = Color(0x10001132);

  static const Color transparent = Color(0x00000000);
  static const Color white = Color(0xFFFFFFFF);
  static const Color white80 = Color(0x80FFFFFF);
  static const Color whitea0 = Color(0xa0FFFFFF);
  static const Color black = Color(0xff333A50);
  static const Color red = Color(0xFFEB5757);

  static const Color border_gray = Color(0xffD6DCE4);
  static const Color button_disabled = Color(0xffbfbfbf);

  static const Color trans_red = Color(0x1aEC3944);
  static const Color trans_green = Color(0x1a22C29B);
  static const Color text_black = Color(0xFF000000);
  static const Color text_red = Color(0xFFEC3944);
  static const Color text_green = Color(0xFF22C29B);
  static const Color text_green_light = Color(0xFF4EDDB6);
  static const Color light_red = Color(0xFFFCF1ED);
  static const Color light_red_text = Color(0xFFA65F3D);
  static const Color light_blue = Color(0xFFEEF5FF);
  static const Color light_blue_text = Color(0xFF36547E);

  static const Color default_line = Color(0xFFEBEBEB);
  static const Color text_disabled = Color(0xFFD4E2FA);
  static const Color selected_item_color = Color(0xFF121826);
  static const Color unselected_item_color = Color(0xFF6C7278);
}
