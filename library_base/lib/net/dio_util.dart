...
class DioUtil {
  ...
  Future<BaseEntity<T>> _request<T>(String url, String method,
    ...
    try {
      ...
      if (params != null) {
        LogUtil.v('请求参数: $params', tag: _TAG);
      }
      if (response != null) {
        LogUtil.v('返回值: $response', tag: _TAG);
      }
    } on DioException catch (e, s) {
      LogUtil.e('请求异常: $e\n$s', tag: _TAG);
      HttpError httpError = HttpError.dioError(e);
      return BaseEntity<T>.fromJson({
        Apis.CODE: httpError.code,
        Apis.MESSAGE: httpError.message,
      });
    } catch (e, s) {
      LogUtil.e("未知异常：$e\n$s", tag: _TAG);
      return BaseEntity<T>.fromJson({
        Apis.CODE: Apis.ERRNO_UNKNOWN,
    ...
    try {
      ...
      final Map<String, dynamic> dataMap = isCompute
          ? await compute(parseData, jsonString)
  ...
  Future requestNetwork<T>(String url, String method,
    ...
    return _request<T>(
      ...
      if (result.code == Apis.SUCCESS_OK) {
        if (onSuccess != null) {
          onSuccess(result.data);
        }
      } else {
        if (result.code == Apis.ERRNO_FORBIDDEN) {
