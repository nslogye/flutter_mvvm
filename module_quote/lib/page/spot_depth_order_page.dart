...
class SpotDepthOrderPage extends StatefulWidget {
  ...
  @override
  SpotDepthOrderPageState createState() => SpotDepthOrderPageState();
}

class SpotDepthOrderPageState extends State<SpotDepthOrderPage>
    with
        BasePageMixin<SpotDepthOrderPage>,
        AutomaticKeepAliveClientMixin<SpotDepthOrderPage>,
  ...
  Widget build(BuildContext context) {
    ...
    return ProviderWidget<SpotDepthModel>(
        ...
        builder: (context, model, child) {
          Widget refreshWidget = const FirstRefreshTop();
          Widget emptyWidget = const LoadingEmptyTop();
                                ...
                                delegate: SliverChildBuilderDelegate(
                                  (context, i) {
                                    return DepthOrderItem(
                                      bid: i < _spotDepthModel.bidsList!.length
                                          ? _spotDepthModel.bidsList![i]
                                          : null,
                                      ask: i < _spotDepthModel.asksList!.length
                                          ? _spotDepthModel.asksList![i]
                                          : null,
                                      bidAmountMax:
                                          _spotDepthModel.bidAmountMax,
                                      askAmountMax:
                                          _spotDepthModel.askAmountMax,
                                    );
                                  },
                                  childCount:
                                      min(20, _spotDepthModel.bidsList!.length),
                                ),
                              ),
                            ],
                          )),
        ...
        children: [
          Expanded(
            child: Container(
                ...
                child: Text(S.of(context).proBid,
          ...
          Expanded(
            child: Container(
                alignment: Alignment.centerLeft,
                child: Text(S.of(context).proAsk,
                    style: TextStyles.textGray500_w400_12,
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis)),
          ),
