...
class _IndexCirculationBarState extends State<IndexCirculationBar> {
  ...
  Widget build(BuildContext context) {
    ...
    num total_supply_market_vol =
        widget.indexData?.total_supply_market_vol ?? 0;

    num turnover = widget.indexData?.turnover ?? 0;
    num turnover_market_vol = total_supply == 0
        ? 0
        : NumUtil.getNumByValueDouble(
            NumUtil.multiply(
                NumUtil.divide(total_supply_market_vol, total_supply),
                turnover),
            0)!;

    num percent = total_supply == 0
        ? 0
        : NumUtil.getNumByValueDouble(
                NumUtil.divideDec(turnover, total_supply).toDouble(), 2)!
            .clamp(0.0, 1.0);
    num lp = total_supply == 0
        ? 0
        : NumUtil.divideDec(turnover, total_supply).toDouble().clamp(0.0, 1.0) *
            100;
    num rp = total_supply == 0
        ? 0
        : NumUtil.divideDec(total_supply - turnover, total_supply)
                .toDouble()
                .clamp(0.0, 1.0) *
            100;
    String lpStr =
        NumUtil.getNumByValueDouble(lp.toDouble(), 2).toString() + "%";
    String rpStr =
        NumUtil.getNumByValueDouble(rp.toDouble(), 2).toString() + "%";
        ...
        : Container(
            ...
            children: [
              Container(
                ...
                child: Text(
                  S.of(context).proCirculationRatio,
              ...
              Container(
                alignment: Alignment.topLeft,
                margin: const EdgeInsets.only(
                    left: 15, top: 13, right: 15, bottom: 18),
                child: Row(
                  children: [
                    Container(
                      width: 4,
                    ...
                    Container(
                      child: Text(
                        '$total_supply$pair / \$$total_supply_market_vol',
                        style: TextStyles.textGray800_w400_12,
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
