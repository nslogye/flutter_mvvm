...
class IndexDetailModel extends ViewStateModel {
  ...
  IndexDetailModel(List<String>? titles) : super(viewState: ViewState.first) {
  ...
  void listenEvent() {
    ...
    quoteSubscription = Event.eventBus.on<WsEvent>().listen((event) {
      ...
      if (quoteCoin != null &&
          quoteWs.coin_code!.toLowerCase() ==
              quoteCoin!.coin_code!.toLowerCase()) {
        if (!_handleKLine) {
        ...
        if (!_handleKLine) {
          quoteSlideController.number =
              NumUtil.formatNum(quoteWs.quote, point: 2);
  ...
  Future getCoinQuote(String chain) {
    ...
    // return DioUtil.getInstance()!.requestNetwork(Apis.URL_GET_COIN_QUOTE, "get",
...
class IndexKLineHandleModel extends ViewStateModel {
  IndexKLineHandleModel() : super(viewState: ViewState.first) {}
}

class IndexHeaderModel extends ViewStateModel {
  IndexHeaderModel() : super(viewState: ViewState.first) {}
}
